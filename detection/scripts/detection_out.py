...
class obs_def_:
    ...
    def __init__(self, index, clss, p, loc):
        ...
        self.b = 1.0
        (H,W) = (540,960)
        # if self.x0 <= 0 or self.x0 >= W-1 or self.x1 <= 0 or self.x1 >= W-1 or self.x2 <= 0 or self.x2 >= W-1 or self.x3 <= 0 or self.x3 >= W-1:
        #     self.b = 'lowx'
        # if self.y0 <= 0 or self.y0 >= H-1 or self.y1 <= 0 or self.y1 >= H-1 or self.y2 <= 0 or self.y2 >= H-1 or self.y3 <= 0 or self.y3 >= H-1:
        #     self.b = 'lowy'        
    # pixel (x,y)
    def getCenter(self):
...
class defect_detection:
    def __init__(self):
        ...
        # 1 = turn, 2 = ER
        self.action = -1
        self.lin_control = PDController(0.2,0.05) # 0.25, 0
        self.ang_control = PDController(1.8,0.5)
        self.speed_ar = []
        ...
        # model_path = "/home/ara/catkin_ws/src/detection/model/arti_cul.pt"
        model_path = "/home/ara/catkin_ws/src/detection/model/onlyspall.pt"
        # model_path = "/home/ara/catkin_ws/src/detection/model/n_morecul2.pt"
        print('load model.......')
        ...
        # rospy.wait_for_message("/range1", Int32)
        # rospy.wait_for_message("/range2", Int32)
    ...
    def callback(self, image, cloud):
        ...
        # self.vis_pub.publish(clear_marker_array)
        # rospy.sleep(0.5)
            ...
            if box.boxes.conf.item() >= 0.5:
                ...
                if not all_real(x_val, y_val, z_val):
                    ...
                    if found_valid.size > 0:
                        ...
                        # rospy.logerr("found valid: %.2f, %.2f, %.2f",x_val, y_val, z_val)
                    else:
                ...
                if z > self.z_constraint or x > 3.5 or x < 1.4 or y >1.7 or y <-1.7:
                    continue
                # debug
                color = (0,255,0)
    ...
    def mainCb(self,event=None):        
        ...
        if self.vis_reach:
            self.vis_pub.publish(self.reach_ar)
        # return
        if (rospy.Time.now() - self.begin).to_sec() < 10:
            rospy.logwarn("waiting: %d", (rospy.Time.now() - self.begin).to_sec())
            return
        ...
        # return
        # return
        if len(self.def_index_black_list) > 2: 
            return
        # GO BACK TO OG if there's nothing left
        # if (len(self.list_def_state) == 0 or len(self.list_def_state) <= len(self.def_index_black_list)) and self.frontier.size == 0:
    ...
    def goTo(self, x, y):
        ...
        if angle_diff < 0.25:
            ang_vel =0
            lin_vel = 0.05   #self.lin_control.update(distance)
